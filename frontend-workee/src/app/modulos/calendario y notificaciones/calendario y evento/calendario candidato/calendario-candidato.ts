...
export class CalendarioCandidatoComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.usuarioService.getIdPorCorreo(correo).subscribe({
      next: (id: number) => { this.idUsuariodelCandidato = id; this.cargarEventos(); },
      error: (err) => console.error('Error obteniendo ID del candidato', err)
    });
  }

  cargarEventos(): void {
    this.calendarioEventoService.getEventosPorUsuario(this.idUsuariodelCandidato).subscribe({
      next: (data) => {
        this.options = {
          ...this.options,
      ...
      error: (err) => console.error('Error cargando eventos candidato', err)
    });
  }
